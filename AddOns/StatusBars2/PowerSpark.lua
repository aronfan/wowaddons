...
local PowerSparkDB = {
	default = {
		...
		parent = StatusBars2_playerPowerBar,
	},
	druid = {
...
function PowerSparkFrame:init(power, last)
	...
	power.spark:SetWidth(8)
	power.spark:SetHeight(24)
...
function PowerSparkFrame:flash(power) --闪动
	...
	if power.interval > 2 then --5秒等待回蓝
		if power.timer + power.interval > GetTime() then
			...
			power.spark:SetPoint('CENTER', StatusBars2_playerPowerBar, 'LEFT', power.bar:GetWidth() - power.bar:GetWidth() * (mod(GetTime() - power.timer, power.interval) / power.interval), 0)
		else --5秒后等待恢复
			power.spark:SetAlpha(0)
		end
	else
		...
		if power.timer + power.interval > GetTime() then
			power.spark:SetPoint('CENTER', StatusBars2_playerPowerBar, 'LEFT', power.bar:GetWidth() * (mod(GetTime() - power.timer, power.interval) / power.interval), 0)
		else --满能量
			power.spark:SetPoint('CENTER', StatusBars2_playerPowerBar, 'LEFT', power.bar:GetWidth() * (mod(math.fmod(GetTime() - power.timer, 2), 2) / 2), 0)
		end
	end
end
